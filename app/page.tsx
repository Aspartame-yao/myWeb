    ...
    <section className="works" id="works" ref={section}>
      ...
      <div className="grid">
        {Array.from({ length: 44 }, (_, index) => {
          ...
          const src = `/tube/im${(index % 5) + 1}.jpg`;
          return (
          <figure className="grid__item" key={`${src}-${index}`}>
            <button className="grid__item-imgwrap" onClick={() => onSelect(project)} aria-label={`Open ${project.title} project details`}>
              <span className="grid__item-img" style={{ backgroundImage: `url(${src})` }} />
              <span className="grid-card-hover"><strong>{project.title}</strong><small>Click to explore ↗</small></span>
            </button>
          </figure>
          );
        })}
  ...
  return (
    <main>
      ...
      <section className="statement" id="about">
        <p>About MGLAB</p>
        <h2>A small laboratory for turning ambiguous ideas into products people can see, touch and use.</h2>
      </section>
      <footer id="contact">
        <p>Have an idea worth making real?</p>
        <a href="mailto:wangmengarc@163.com">wangmengarc@163.com</a>
        <span>© 2026 MGLAB</span>
      </footer>
